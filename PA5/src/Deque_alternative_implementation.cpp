...
void push_back(Deque& deque, int val){
    ...
    else if (deque.end.current != deque.end.last){
...
void pop_back(Deque& deque){
    ...
    else if ((deque.end.current != deque.end.first + 1) || ((deque.end.current == deque.end.first + 1) && deque.ll_size == 1)){
        deque.end = prev(deque.end);
    }
    else if ((deque.end.current == deque.end.first + 1) && deque.ll_size > 1){
        Iterator working_iter = end(deque);
        deque.end.current = deque.end.node->prev->arr + CHUNK_SIZE;
...
void print_deque(const Deque& deque){
    ...
    while (working_iter.current != deque.end.current){
        if (working_iter.current != working_iter.last){
            ...
            else if ((working_iter.current != deque.end.current - 1)) cout << ", ";

            if (working_iter.current != working_iter.last) working_iter.current++;
            else {
...
void insert(Deque& deque, const Iterator& pos, int val){
    ...
    if (deque.end.current == deque.end.last){ // there's no space to accommodate the newly inserted number - append one new node
        Node* new_node = new Node;
        new_node->next = deque.sentinel;
        new_node->prev = deque.sentinel->prev;
        deque.sentinel->prev = new_node;
    ...
    else deque.end = next(deque.end);
    while (working_iter.current != pos_ref.current){
        if (working_iter.current == working_iter.last - 1){
            *(working_iter.node->next->arr) = *(working_iter.current - 1);
        }
        else if (working_iter.current == working_iter.last){;}
        else {
...
void erase(Deque& deque, const Iterator& pos){
    ...
    Iterator pos_ref = pos;
    while (working_iter.current != deque.end.current - 1){
        if (working_iter.current != working_iter.last - 1) {
            *(working_iter.current) = *(working_iter.current + 1);
            working_iter.current++;
        }
    ...
    if ((deque.end.current == deque.end.first + 1) && deque.ll_size > 1){
        set_node(deque.end, deque.end.node->prev);
